...
// --- GÜVENİLİR & YÜKSEK ÇÖZÜNÜRLÜKLÜ MENÜ VERİLERİ ---
const MENU_ITEMS = [
  {
    id: '1',
  ...
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1592417817098-8f3d6eb12765?auto=format&fit=crop&w=1200&q=80',
...
export default function AvantGardeGastronomyPage() {
  ...
  // İnteraktif Rezervasyon Konfigüratör Eyaletleri
  ...
  const openWhatsApp = () => {
    const text = encodeURIComponent(`Merhaba L'Étoile Noir Concierge, ${seatingArea} alanı için ${guestCount} kişilik VIP rezervasyon bilgisi almak istiyorum.`);
  ...
  return (
    <main className="min-h-screen bg-[#020204] text-slate-100 font-sans selection:bg-amber-400 selection:text-black relative overflow-x-hidden">
      ...
      {/* Hero Section */}
      <section className="relative min-h-[80vh] flex items-center justify-center px-6 md:px-12 overflow-hidden border-b border-amber-500/10">
          ...
          <AnimatePresence mode="popLayout">
            {filteredMenu.map((item) => (
              <motion.div
                ...
                className="group rounded-3xl bg-slate-950/80 border border-amber-500/20 hover:border-amber-500/60 overflow-hidden flex flex-col justify-between transition-all duration-500 hover:shadow-[0_0_50px_rgba(245,158,11,0.2)] backdrop-blur-xl relative"
              >
                <div>
                  {/* Görsel Kutusu */}
                  <div className="relative h-72 overflow-hidden bg-slate-900">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-tr from-amber-950 via-slate-900 to-slate-950 flex items-center justify-center p-6 text-center">
                      <span className="text-xs font-mono text-amber-400/50 uppercase tracking-widest">{item.name}</span>
                    </div>

                    <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/20 to-transparent opacity-90 z-20" />
                    
                    {/* Rozet */}
                    <div className="absolute top-4 left-4 z-30 px-3.5 py-1.5 rounded-full bg-slate-950/80 border border-amber-500/40 text-amber-300 text-[10px] font-mono font-bold tracking-wider uppercase backdrop-blur-md shadow-lg">
                      {item.badge}
                    </div>
                    ...
                    <div className="absolute bottom-4 right-4 z-30 text-3xl font-black text-amber-400 font-mono drop-shadow-[0_4px_10px_rgba(0,0,0,0.9)]">
                      {item.price}
                    </div>
                  </div>
                ...
                <div className="p-7 pt-0 space-y-4">
                  <div className="p-3.5 rounded-2xl bg-slate-900/80 border border-slate-800/80 flex items-center justify-between text-xs font-mono">
                  ...
                  <button 
                    ...
                    className="w-full py-3.5 rounded-2xl bg-slate-900 hover:bg-amber-500 hover:text-slate-950 border border-slate-800 hover:border-amber-500 text-slate-200 text-xs font-mono font-bold transition-all flex items-center justify-center gap-2 group-hover:shadow-[0_0_20px_rgba(245,158,11,0.3)]"
                  >
      ...
      {/* ========================================================== */}
      {/* YENİ NESİL, UÇUK VE DİKKAT ÇEKİCİ HOLOGRAFİK REZERVASYON KONSOLU */}
      {/* ========================================================== */}
      <section className="py-24 px-6 md:px-12 max-w-6xl mx-auto relative z-10">
        <div className="relative rounded-[40px] bg-[#040408] border border-amber-500/40 p-8 md:p-16 overflow-hidden shadow-[0_0_120px_rgba(245,158,11,0.18)] backdrop-blur-3xl">
          
          {/* Gelecekçi Arka Plan Tarama Çizgileri ve Işık Efektleri */}
          ...
          <div className="max-w-4xl mx-auto space-y-12 relative z-10">
            ...
            {/* Konsol Üst Başlık & Sistem Durumu */}
            <div className="flex flex-col md:flex-row items-center justify-between border-b border-amber-500/20 pb-8 gap-6 text-center md:text-left">
            ...
            {/* Salon / Alan Seçici Kartlar (Gelişmiş Holo Stili) */}
                  ...
                  <button
                    ...
                    className={`group relative p-6 rounded-3xl border transition-all duration-500 text-left overflow-hidden ${
                      seatingArea === area.name
                        ? 'bg-gradient-to-b from-amber-500/20 to-black/80 border-amber-400 shadow-[0_0_35px_rgba(245,158,11,0.35)] scale-[1.02]'
                        : 'bg-black/40 border-slate-800/80 text-slate-400 hover:border-amber-500/40 hover:bg-black/60'
                    }`}
                  >
                    ...
                    <Compass className={`w-6 h-6 mb-4 transition-colors ${seatingArea === area.name ? 'text-amber-400' : 'text-slate-500 group-hover:text-amber-400'}`} />
                    <div className="text-sm font-bold font-serif text-white mb-1">{area.name}</div>
                    <div className="text-[11px] text-slate-400 font-light leading-relaxed">{area.desc}</div>
                    
                    {seatingArea === area.name && (
                      <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-amber-500 to-amber-300 animate-pulse" />
                    )}
                  </button>
                ))}
              </div>
            ...
            {isReserved ? (
              <motion.div
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                className="p-10 rounded-3xl bg-emerald-950/40 border-2 border-emerald-500/60 text-emerald-300 font-mono space-y-4 text-center shadow-[0_0_50px_rgba(16,185,129,0.3)]"
              >
              ...
              <form onSubmit={handleReservation} className="space-y-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full px-6 py-4 rounded-2xl bg-black/80 border border-slate-800 text-white text-sm focus:outline-none focus:border-amber-400 focus:shadow-[0_0_20px_rgba(245,158,11,0.25)] transition-all placeholder:text-slate-600"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="block text-xs font-mono uppercase tracking-[0.2em] text-slate-400 font-bold">
                ...
                {/* Tarih, Saat ve Kişi Sayısı Seçicileri */}
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      type="date"
                      required
                      className="w-full px-6 py-4 rounded-2xl bg-black/80 border border-slate-800 text-white text-sm focus:outline-none focus:border-amber-400 focus:shadow-[0_0_20px_rgba(245,158,11,0.25)] transition-all"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="block text-xs font-mono uppercase tracking-[0.2em] text-slate-400 font-bold">
                ...
                <div className="pt-4">
                  <button
                    type="submit"
                    className="group relative w-full py-6 rounded-2xl bg-gradient-to-r from-amber-600 via-amber-400 to-amber-600 text-slate-950 font-black text-sm uppercase tracking-[0.25em] shadow-[0_0_40px_rgba(245,158,11,0.4)] hover:shadow-[0_0_70px_rgba(245,158,11,0.8)] hover:scale-[1.01] transition-all duration-300 flex items-center justify-center gap-3 overflow-hidden"
                  >
                    <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
                    <Zap className="w-5 h-5 fill-slate-950 relative z-10" />
                    <span className="relative z-10">Kuantum Kod ile Anında Rezerve Et</span>
                    <ArrowUpRight className="w-5 h-5 relative z-10 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
                  </button>
                  <p className="text-center text-[10px] font-mono text-slate-500 mt-4 tracking-wider uppercase">
                    İşleminiz 256-bit şifreli siber güvenlik protokolleri ile korunmaktadır.
                  </p>
                </div>
              </form>
            )}
      ...
      {/* Floating Canlı WhatsApp / VIP Concierge Butonu */}
      <div className="fixed bottom-8 right-8 z-50">
        <button
          onClick={openWhatsApp}
          className="group relative flex items-center gap-3 px-5 py-3.5 rounded-full bg-emerald-500 text-slate-950 font-bold text-xs shadow-[0_0_30px_rgba(16,185,129,0.5)] hover:bg-emerald-400 hover:scale-105 transition-all duration-300"
        >
          ...
          <span className="font-mono tracking-wider uppercase">VIP Concierge Hat (WhatsApp)</span>
        </button>
      </div>
      ...
      <AnimatePresence>
        {selectedImage && (
          <motion.div 
            ...
            className="fixed inset-0 z-50 bg-black/95 backdrop-blur-2xl flex items-center justify-center p-6 cursor-pointer"
          >
            <div className="relative max-w-6xl w-full max-h-[90vh] rounded-3xl overflow-hidden border border-amber-500/40 shadow-[0_0_100px_rgba(245,158,11,0.3)]">
              <img src={selectedImage} alt="Ultra Detail" className="w-full h-full object-contain" />
              <button 
                onClick={() => setSelectedImage(null)}
                className="absolute top-6 right-6 p-4 rounded-full bg-slate-950/80 border border-white/20 text-white hover:bg-amber-500 hover:text-black transition-all"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
      ...
      {/* Footer */}
      <footer className="py-12 border-t border-slate-900 text-center text-xs font-mono text-slate-500 relative z-10">
